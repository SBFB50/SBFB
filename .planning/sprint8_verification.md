...
| 11 | Python — SDK db tests | `uv run pytest packages/nexus-sdk/tests/test_db.py -q` | ≥ 8 pass | ✅ **8 passed** (path resolution, fetchall, fetchone, parameterized query, missing file → IOError, concurrent fetchall under asyncio.Lock, read-only enforced via `mode=ro`, schema introspection) |
