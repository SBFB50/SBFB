...
- [~] **DHT redundant lookup** : primitive `redundant_resolve` + `QuorumResolver` trait + 13 tests verts (Phase C `9d0ad7a`). **Wiring runtime** au browse aggregator + curator runtime carry-over Sprint 19 (HARDENING_ROADMAP §S19 PoW gossip + TLS pinning), tracke par audit S18 finding C-1 (P2). Demarche : la primitive est generic-over-resolver et testee, le wire au probe_reachable demande deep iroh-relay 0.97 API knowledge pour exposer un per-relay lookup utilisable comme `QuorumResolver`. Reportee a S19 qui touche au transport P2P.
