...
- [x] **DHT redundant lookup** : primitive `redundant_resolve` + `QuorumResolver` trait + 13 tests verts (Phase C `9d0ad7a`). **Wiring runtime** livre Sprint 19 Phase A `ab6985c` (carry S18 C-1 resolu) — `PkarrQuorumResolver` + `PkarrRelayClient` wrap cables au browse aggregator + curator runtime via canary opt-in `SBFB_PKARR_RELAYS` env var (disabled par defaut). Eclipse-by-DHT defense **runtime-active sous config** ; enforcement strict par defaut reste post-Gate 2. Cf. `.planning/active/sprint19_audit_findings.md §Track A` finding P2-A1 (claim-drift « pleinement active » reclasse en « runtime-active sous config ») + `sprint19_phase_A_review.md` + `sprint19_verification.md §Gate HARDENING_ROADMAP §3 S19`.
