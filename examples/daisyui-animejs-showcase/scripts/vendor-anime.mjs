...
//
// Why UMD (classic script) and NOT ESM: the iframe runs at an OPAQUE origin
// (sandbox without allow-same-origin) under COEP require-corp. A
// `<script type="module">` is fetched in CORS mode, which an opaque-origin
// document cannot satisfy for its own assets. A classic `<script src>` is
// no-cors and loads under `default-src 'self'`. This mirrors every shipped
// SBFB app (sbfb-ideas, sbfb-explorer): external CSS via <link> + classic JS.
// The UMD bundle exposes a single global `anime` with all named members.
...
const header =
  `/* anime.js v${pkg.version} — MIT (c) Julian Garnier — github.com/juliangarnier/anime\n` +
