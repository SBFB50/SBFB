...
struct RegistryState {
    /// `project_id -> (seeder_node_id -> last_seen_unix_secs)`.
    seeders: HashMap<String, HashMap<String, u64>>,
...
/// project bucket left empty.
fn sweep_expired(seeders: &mut HashMap<String, HashMap<String, u64>>, now: u64) {
...
impl SeedRegistry {
    ...
    /// Record that `seeder_node_id` announced it seeds `project_id` at
    /// `seen_at` (unix secs). Idempotent per (project, seeder): a repeat
    /// announcement only refreshes the last-seen timestamp, and a stale
    /// replayed ts never overwrites a fresher observation. Runs a throttled
    /// global sweep (every `SEED_SWEEP_INTERVAL_SECS` of announcement time) so
    /// the map cannot grow unbounded for projects that are never queried.
    pub fn record(&self, project_id: &str, seeder_node_id: &str, seen_at: u64) {
        ...
        let seeders = state.seeders.entry(project_id.to_string()).or_default();
        seeders
    ...
    pub fn record_announced(
        ...
        self.record(&p.project_id, &p.seeder_node_id, seen_at);
        true
    }

    /// Count distinct seeders of `project_id` seen within the TTL at
    /// `now` (unix secs). Lazily evicts expired entries (and drops an
    /// emptied project bucket) so churned projects do not leak memory.
    pub fn count_recent(&self, project_id: &str, now: u64) -> usize {
        let mut state = self.inner.lock().unwrap_or_else(|p| p.into_inner());
        let cutoff = now.saturating_sub(SEED_SEEN_TTL_SECS);
        let Some(seeders) = state.seeders.get_mut(project_id) else {
            return 0;
        };
        seeders.retain(|_, ts| *ts >= cutoff);
        let count = seeders.len();
        if count == 0 {
            state.seeders.remove(project_id);
        }
        count
    }

    /// The distinct seeder ids of `project_id` seen within the TTL at `now`,
    /// sorted for determinism. Same lazy-purge semantics as
    /// [`Self::count_recent`]; used by tests to assert WHICH seeder was
    /// recorded (not just the count), so a mutation that corrupts the stored
    /// `seeder_node_id` is caught instead of passing on the count alone.
    /// Test-only introspection: prod reads the count via `count_recent`.
    #[cfg(test)]
    pub fn seeders_recent(&self, project_id: &str, now: u64) -> Vec<String> {
        let mut state = self.inner.lock().unwrap_or_else(|p| p.into_inner());
        let cutoff = now.saturating_sub(SEED_SEEN_TTL_SECS);
        let Some(seeders) = state.seeders.get_mut(project_id) else {
            return Vec::new();
    ...
    /// Number of project buckets currently resident (test-only — asserts the
    /// global sweep actually bounds memory for never-queried projects).
    #[cfg(test)]
...
mod tests {
    ...
    fn seed_announced_ingested_increments_count() {
        ...
        assert_eq!(reg.count_recent(&pid, now + 10), 2);
    ...
    fn seed_count_best_effort_ttl_expires() {
        ...
        reg.record(&pid, &fresh, now);
        reg.record(&pid, &stale, now - SEED_SEEN_TTL_SECS - 1);

        // The stale peer is evicted on read; only the fresh one counts.
        assert_eq!(reg.count_recent(&pid, now), 1);

        // Once every peer expires the project bucket is dropped (count 0).
        assert_eq!(reg.count_recent(&pid, now + SEED_SEEN_TTL_SECS + 2), 0);

        // An unknown project is simply zero, never a panic.
        assert_eq!(reg.count_recent(&"f".repeat(64), now), 0);
    }
    ...
    fn seed_registry_global_sweep_bounds_memory() {
        ...
        reg.record(&"a".repeat(64), &"11".repeat(32), t0);
        reg.record(&"b".repeat(64), &"22".repeat(32), t0);
        ...
        reg.record(&"c".repeat(64), &"33".repeat(32), later);

        // A and B were swept; only the fresh project C remains resident.
        assert_eq!(reg.project_count(), 1);
        assert_eq!(reg.count_recent(&"c".repeat(64), later), 1);
    }
}
