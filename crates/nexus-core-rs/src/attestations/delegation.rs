...
use crate::canonical::{canonical_bytes, DOMAIN_DELEGATION_CERT_V1};
...
mod tests {
    ...
    fn rejects_invalid_algo() {
        let kp = KeyPair::generate();
        let err =
            DelegationCert::sign("pgp-rsa-4096", SAMPLE_FINGERPRINT, 1_713_600_000, None, &kp)
                .expect_err("bad algo");
    ...
    fn rejects_uppercase_fingerprint() {
        ...
        let err =
            DelegationCert::sign(DELEGATION_ALGO_SSH_ED25519, upper, 1_713_600_000, None, &kp)
                .expect_err("bad fp");
