...
export function ConformiteCard({ rev }: { rev: string }) {
  ...
  if (error) return <div className="font-mono text-meta text-warn">{error}</div>
  if (audit === null) return <div className="font-mono text-meta text-tx4">audit de conformité…</div>
    ...
    <div data-testid="conformite-card" className="flex flex-col gap-3 rounded-md border border-bd bg-s1 px-4 py-3">
      <div className="flex items-center gap-2 font-mono text-meta">
        ...
        {audit.is_phase_commit ? (
          <span className="rounded-sm border border-bd2 px-1 py-0.5 text-meta text-tx3">phase</span>
        ) : (
          <span className="rounded-sm border border-bd px-1 py-0.5 text-meta text-tx3">hors-phase</span>
        )}
      </div>
      ...
      <div>
        ...
        {audit.issues.length === 0 ? (
          <div className="font-mono text-meta text-tx3">0 manque relevé par l'audit Rust</div>
        ) : (
          <ul className="flex flex-col gap-1">
            {audit.issues.map((issue, i) => (
              <li key={i} className="flex items-start gap-2 font-mono text-meta text-warn">
      ...
      {lint ? (
        <div className="border-t border-bd pt-2">
          ...
          {lint.errors.length === 0 && lint.warnings.length === 0 ? (
            <div className="font-mono text-meta text-tx3">0 diagnostic</div>
          ) : (
            <ul className="flex flex-col gap-1">
              {lint.errors.map((d, i) => (
                <li key={`e${i}`} className="font-mono text-meta text-bad">
                  [{d.code}] {d.message}
                  {d.file ? <span className="text-tx4"> · {d.file}</span> : null}
                </li>
              ))}
              {lint.warnings.map((d, i) => (
                <li key={`w${i}`} className="font-mono text-meta text-warn">
                  [{d.code}] {d.message}
                  {d.file ? <span className="text-tx4"> · {d.file}</span> : null}
                </li>
              ))}
