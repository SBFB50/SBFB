...
function Journal({ entries }: { entries: ActionLogEntry[] }) {
  if (entries.length === 0) return <div className="font-mono text-meta text-tx4">aucune action enregistrée</div>
  return (
    <ul className="flex flex-col gap-0.5" data-testid="action-journal">
      {entries.map((e, i) => (
        <li key={i} className="flex items-baseline gap-2 font-mono text-meta">
...
function ChatMessage({ role, content }: { role: string; content: string }) {
  ...
  return (
    <div className="rounded-sm border border-bd bg-s1 px-2.5 py-1.5">
      <span className="mr-2 font-mono text-meta text-tx4">{role}</span>
      <span className="font-sans text-body text-tx2">{shown}</span>
      {long ? (
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="ml-1.5 font-mono text-meta text-info hover:underline"
...
function ResumePanel({ sessions }: { sessions: ClaudeSession[] }) {
  ...
  if (sessions.length === 0) {
    return <div className="font-mono text-meta text-tx4">aucune session claude reprenable pour ce dépôt</div>
  }
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-1" data-testid="claude-sessions">
        {sessions.map((s) => (
          ...
          >
            <span className="shrink-0 rounded-sm border border-bd px-1 text-meta text-tx3">claude</span>
            <span className="min-w-0 flex-1 truncate font-sans text-sec text-tx2">{s.name || s.session_id}</span>
            <span className="shrink-0 font-mono text-meta text-tx4">{formatSessionDate(s.updated_at)}</span>
            <button
              type="button"
              onClick={() => toggleResume(s.session_id)}
              data-testid="claude-resume"
              className={`shrink-0 rounded-sm border px-2 py-0.5 font-mono text-meta ${
      ...
      {resume ? (
        <div className="flex flex-col overflow-hidden rounded-md border border-bd" data-testid="resume-terminal">
          <div className="flex items-center gap-2 border-b border-bd bg-s1 px-3 py-1 font-mono text-meta text-tx3">
          ...
          <div className="h-72 min-h-0 bg-s0 p-2">
            <Suspense fallback={<div className="p-3 font-mono text-meta text-tx4">chargement du terminal…</div>}>
    ...
    <div data-testid="sessions-surface" className="flex min-h-0 flex-1 flex-col overflow-auto p-5">
      <section className="mb-5">
        ...
        {error ? (
          ...
          <div className="font-mono text-meta text-tx4">lecture du journal…</div>
        ) : (
          <Journal entries={log} />
        )}
      </section>
      ...
      <section className="mb-5">
        ...
        {claudeSessions === null ? (
          <div className="font-mono text-meta text-tx4">lecture des sessions…</div>
        ) : (
      ...
      <section className="min-h-0 flex-1">
        ...
        {casts === null ? (
          ...
          <div className="font-mono text-meta text-tx4">aucun enregistrement</div>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {casts.map((c) => (
              <button
                ...
                className={`rounded-sm border px-2 py-1 font-mono text-meta ${
