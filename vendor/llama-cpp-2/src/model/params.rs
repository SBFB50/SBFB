...
impl LlamaModelParams {
    ...
    ///
    /// Load ONLY the weights of the transformer layers in `[start, end)`; the rest are
    /// `TENSOR_SKIP`-ped so this node allocates VRAM for its shard only. `end == 0` means
    /// `n_layer`. The token embedding stays resident on every shard (the graph builder
    /// references it unconditionally); `is_last` additionally keeps the output norm + lm_head.
    /// The defaults `(0, 0, true, true)` load the whole model (upstream behaviour). This MUST
    /// match the context params shard range set via
    /// [`crate::context::params::LlamaContextParams::with_shard_range`].
    ///
    /// # Examples
    ///
    /// ```
    /// # use llama_cpp_2::model::params::LlamaModelParams;
    /// let params = LlamaModelParams::default().with_shard_range(2, 16, false, true);
    ...
    pub fn with_shard_range(mut self, start: i32, end: i32, is_first: bool, is_last: bool) -> Self {
        self.params.shard_layer_start = start;
        self.params.shard_layer_end = end;
        self.params.shard_is_first = is_first;
        self.params.shard_is_last = is_last;
        self
    }
    ...
    pub fn shard_range(&self) -> (i32, i32, bool, bool) {
        (
            self.params.shard_layer_start,
            self.params.shard_layer_end,
            self.params.shard_is_first,
            self.params.shard_is_last,
        )
    }
}
