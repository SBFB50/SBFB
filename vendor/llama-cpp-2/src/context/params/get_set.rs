...
impl LlamaContextParams {
    ...
    /// Set the SBFB pipeline-split shard window (Sprint 77 fork).
    ///
    /// Execute ONLY the transformer layers in `[start, end)`; `end == 0` means `n_layer`.
    /// An intermediate shard (`is_last == false`) exposes the RAW residual stream at the
    /// boundary via [`crate::context::LlamaContext`] embeddings (no final norm, no
    /// lm_head); the next shard resumes by injecting it as the batch `embd`. `is_first`
    /// has no effect on the execution graph (the injection path is selected at runtime by
    /// whether the batch carries tokens or embeddings) but is kept for symmetry with the
    /// model params window. MUST be set BEFORE `model.new_context(...)` so the buffer
    /// reserve uses it, and MUST match the model params window set via
    /// [`crate::model::params::LlamaModelParams::with_shard_range`]. Defaults
    /// `(0, 0, true, true)` execute the whole model (upstream behaviour).
    ///
    /// # Examples
    ///
    /// ```
    /// # use llama_cpp_2::context::params::LlamaContextParams;
    /// let params = LlamaContextParams::default().with_shard_range(2, 16, false, true);
    /// assert_eq!(params.shard_range(), (2, 16, false, true));
    /// ```
    #[must_use]
    pub fn with_shard_range(mut self, start: i32, end: i32, is_first: bool, is_last: bool) -> Self {
        self.context_params.shard_layer_start = start;
        self.context_params.shard_layer_end = end;
        self.context_params.shard_is_first = is_first;
        self.context_params.shard_is_last = is_last;
        self
    }

    /// Get the SBFB pipeline-split shard window `(start, end, is_first, is_last)`.
    #[must_use]
    pub fn shard_range(&self) -> (i32, i32, bool, bool) {
        (
            self.context_params.shard_layer_start,
            self.context_params.shard_layer_end,
            self.context_params.shard_is_first,
            self.context_params.shard_is_last,
        )
    }
}
