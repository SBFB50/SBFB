...
last_validated: 2026-07-15  # S82 Phase K — bump hickory-resolver 0.24→0.26 JOUE (carry HICKORY-024-RUSTSEC solde) : RUSTSEC-2026-0119 (hickory-proto 0.26.1, seuil exact 0.26.1, DoS) + 0098/0099 (rustls-webpki 0.103.13, laxite name-constraints, classe AUTHENTIFICATION) + 0104 (panic CRL, seuil dur 0.103.13) FERMEES, 4 ignores deny.toml RETIRES (les 2 quick-xml via iroh RESTENT, upstream). Arbre 0.24 legacy (rustls 0.21.12, rustls-webpki 0.101.7, tokio-rustls 0.24.1) sorti NET du lock (collapse 2→1 versions). Backend crypto ring CONSERVE (aucun aws-lc-rs, grep lock = 0) ; webpki-roots EXPLICITE (0.26 defaut = aucun root store). Remediation yanked meme phase : spin 0.9.8/0.10.0 → 0.9.9/0.10.1 (lock-only). P2-AUDIT-2-RESIDUEL (multiple-versions warn, ed25519-dalek RC iroh) INCHANGE. Precedent 2026-07-08 S81 Phase G — trigger « iroh release > 0.98 » FIRED et JOUE : upgrade stack iroh =1.0.1 / docs 0.101.0 / gossip 0.101.0 / blobs 0.103.0 (S81 Phases B-F). Gate convergence supply-chain NON convergent (ed25519-dalek 2.2.0 SBFB-stable + 3.0.0-rc.0 interne iroh + curve25519 5.0.0-rc.0 → P2-AUDIT-2-RESIDUEL carry S82, deny.toml multiple-versions reste warn). Advisories remediees Phase G : cargo update anyhow 1.0.103 + crossbeam-epoch 0.9.20 ; 6 ignore-with-reason (4 racine hickory-resolver 0.24 → carry HICKORY-024-RUSTSEC S82 ; 2 quick-xml via iroh, upstream) ; ignore rand RUSTSEC-2026-0097 perime RETIRE. La note « iroh 1.0.0-rc.0 defere » du precedent est OBSOLETE (upgrade fait). upgrade ≠ Gate 1/Gate 3, R-iroh-audit P0 INCHANGE, pilote reste ferme. Precedent 2026-06-03 S73 Phase A — §3 recadre (backlog S18-30 clos + pointeur threat docs vivants ; P2-HARDENING-ROADMAP-META-STALE).
triggers_revalidate:
  - "iroh breaking release > 1.0.x OU iroh-docs 0.102+ (wire pre-1.0, 2 casses en 6 semaines) OU yank ed25519-dalek 3.0.0-rc.0 (deny.toml yanked=deny casserait le pin exact iroh) — re-arme S81 Phase G apres FIRED (upgrade 0.98→=1.0.1 joue)"
  ...
  - "frost-ed25519 release > 3.0 (upgraded S34 Phase A, next breaking > 3.x)"
  ...
  - "hickory-resolver breaking release > 0.26 OU nouvelle RUSTSEC sur la chaine rustls 0.23 / rustls-webpki / hickory-proto tiree par les features tls-ring/https-ring/webpki-roots (bump 0.24→0.26 joue S82 Phase K)"
audited_findings:
  ...
  - "2026-07-08 S81 Phase G supply-chain + docs securite : gate convergence crypto JOUE apres l'upgrade iroh 0.98→=1.0.1 (B-F) — cargo tree -d : lock NON convergent (ed25519-dalek 2.2.0 [SBFB, pin workspace 2.1, tire par nexus-core-rs + nexus-trace-core] ET 3.0.0-rc.0 [interne iroh/iroh-base 1.0.1] ; curve25519-dalek 4.1.3 ET 5.0.0-rc.0 ; 72 groupes multi-version cargo deny check bans). Flip deny.toml multiple-versions warn→deny IMPOSSIBLE → P2-AUDIT-2-RESIDUEL (carry S82) ; debloque par une release upstream iroh repassant ed25519-dalek 3.x du RC au stable (iroh 1.0.2 du 2026-07-06 pin ENCORE =3.0.0-rc.0, verifie crates.io — bump ne resout pas ; pin SBFB =1.0.1 garde par choix). Crypto SBFB security-critical ISOLEE sur l'arbre stable (aucun chemin SBFB ne linke le RC). cargo deny check advisories ROUGE→VERT : cargo update anyhow 1.0.102→1.0.103 (RUSTSEC-2026-0190) + crossbeam-epoch 0.9.18→0.9.20 (RUSTSEC-2026-0204) ; ignores documentes RUSTSEC-2026-0119/0098/0099/0104 (racine pin hickory-resolver 0.24 — bump 0.26 = churn API dns_fallback.rs, hors mandat iroh-STRICTEMENT-SEUL Day-0 #10 → carry HICKORY-024-RUSTSEC S82 ; classes intrinseques distinctes : 0119/0104 = DoS, 0098/0099 = laxite name-constraints certs [classe authentification, PAS DoS] ; residuel SBFB borne pour les 4 : fallback opt-in default-off, resolveurs epingles Cloudflare/Google TLS-valides, pkarr Ed25519-verifie en aval) + RUSTSEC-2026-0194/0195 (quick-xml via iroh→netwatch→plist, input local macOS, fix upstream, re-check a chaque bump iroh) ; ignore rand RUSTSEC-2026-0097 perime retire (advisory-not-detected, rand 0.8.6 hors plage revisee ; l'exemption d'upgrade rand 0.9 elle-meme inchangee). rust-version 1.91 : deja landee Phase B c899d54 (verification, pas d'edit). TOOLCHAIN-LABEL (S80-A-2) : STATU QUO — pas de rust-toolchain.toml ; arbitre fmt/clippy = Docker canonique rust:1.94 SHA-pin (Woodpecker), toolchain Windows locale FLOTTANTE (>=1.95), GHA @stable, MSRV declaree 1.91 = plancher ; les futurs self-reports doivent etiqueter « Win natif (toolchain flottante) », jamais une version pinnee fictive. Docs securite : THREAT_MODEL v15 (§15.4 zero-n0 + residuels E3/F + note upgrade≠Gate1) + EXTERNAL_AUDIT_SCOPE §2.4/§2.7 (note R-iroh-audit P0 reconfirmee verbatim, checklist rejouee et consignee) + LOOPBACK §3.1 revalide (LOT-LOOPBACK-DOC S80 H-1/2/3/4). Escalade a l'audit gate S81 : carry S75 re-drive-on-ingest boot-SEED driver OVERDUE 3/3 NON ferme (hors mandat G, code non-supply-chain) — fermer ou re-justifier blocker externe."
  - "2026-07-15 S82 Phase K supply-chain hickory : bump hickory-resolver 0.24→0.26 (carry HICKORY-024-RUSTSEC SOLDE). Construction resolver reecrite dns_fallback.rs (nexus-core-rs — le fichier reel, PAS nexus-shell-daemon comme ecrit au kickoff) : Resolver/builder_with_config + NameServerConfig::new + ConnectionConfig::https/tls per-endpoint (TLS name P2-E-1 preserve), trust_negative_responses=false rendu EXPLICITE (defaut upstream bascule a true en 0.26, le caching negatif defairait la course DoH/DoT), extraction TXT via lookup.answers() + RData::TXT, enum local DnsTransport{Doh,Dot} remplace l'enum hickory Protocol supprime (-1 test type-vacant build_resolver_rejects_unsupported_protocol : etat irrepresentable). Features 0.26 : tls-ring + https-ring (les noms 0.24 dns-over-*-rustls sont SUPPRIMES upstream) + webpki-roots OBLIGATOIRE (0.26 defaut = AUCUN root store — omission = echec silencieux de tous les handshakes, invisible aux tests unitaires). 4 advisories FERMEES au lock et ignores deny.toml RETIRES : RUSTSEC-2026-0119 hickory-proto 0.26.1 (seuil exact 0.26.1 — 0.25.x/0.26.0 restent vulnerables), classe DoS name-compression ; RUSTSEC-2026-0098/0099 rustls-webpki 0.103.13 (seuil 0.103.12), classe AUTHENTIFICATION laxite name-constraints (PAS DoS) ; RUSTSEC-2026-0104 rustls-webpki 0.103.13 (seuil dur 0.103.13), panic CRL DoS/availability. Arbre 0.24 legacy sorti NET du lock (hickory-* 0.24.4, rustls 0.21.12, rustls-webpki 0.101.7, tokio-rustls 0.24.1, h2 0.3.27, http 0.2.12 + 5 autres) — collapse 2→1 versions, l'arbre 0.26 etait DEJA present via iroh =1.0.1. Backend crypto ring CONSERVE (aws-lc-rs ABSENT du lock, grep = 0) ; rustls-platform-verifier 0.7.0 pre-existant via iroh-metrics (PAS ajoute par ce bump, webpki-roots choisi pour preserver le comportement 0.24 roots Mozilla deterministes). Remediation yanked meme phase (classe S81-G anyhow/crossbeam) : spin 0.9.8 + 0.10.0 yankes upstream (chaines heapless→postcard→frost et futures-buffered→iroh-docs) → cargo update cible 0.9.9/0.10.1, lock-only. Ignores quick-xml 0194/0195 + exemption rand INCHANGES ; P2-AUDIT-2-RESIDUEL (multiple-versions=warn) INCHANGE."
-->
